		...
		if ($arResult["NAV_STRING_COUNT_MORE"] > 0)
		{
			...
			?><div class="feed-com-header"><?
				?><a class="feed-com-all" href="<?=$arParams["NAV_STRING"]?>" id="<?=$prefixNode?>_page_nav" bx-mpl-comments-count="<?=$arResult["NAV_STRING_COUNT_MORE"]?>"><?
					?><?=($arParams["PREORDER"] == "Y" ? GetMessage("BLOG_C_VIEW1") : GetMessage("BLOG_C_VIEW2"))?><i></i><?
				?></a><?
			?></div><?
			if ($arParams["PREORDER"] != "Y")
			{
...
// если в массиве параметров сообщения параметр ['AUX'] имеет значение: "TASKINFO", то цикл пропускает это сообщение и идет дальше, вывод его не происходит
		if($res['AUX'] == "TASKINFO")
        {
            continue;
        }
// в противном случае производится вывод сообщения
		if ($res["COLLAPSED"] === "Y")
		{
			$collapsedMessages++;
